...
function calHTML(){
  ...
  return '<div class="cal-head"><button class="cal-nav" onclick="calNav(-1)" aria-label="Mois précédent">‹</button><div class="cal-title">'+MN[m]+' '+y+'</div><button class="cal-nav" onclick="calNav(1)" aria-label="Mois suivant">›</button></div>'+
    ...
    '<div class="cal-legend"><span><i class="cal-lg cal-g"></i>Réalisé</span><span><i class="cal-lg cal-o"></i>Non réalisé</span><span><i class="cal-lg cal-x"></i>À venir</span><span style="opacity:.45;margin-left:auto">build 18</span></div>'+_emptyCal;
}
...
function coachAvant(num,se){const c=COACH_THEORY[seanceTag(se)];if(!c)return '';
  const tag=seanceTag(se);const rich=RICH_THEORY[tag]&&String(num)==='25';
  const theoBtn=rich
...
const RICH_THEORY={
 ef:{titre:'Endurance fondamentale',icone:'🌱',sections:[
   ...
   coach:`<p>Loïc, sur tes footings de reprise (S25), l'objectif n'est pas la distance ni l'allure : c'est de <strong>réinstaller le réflexe « facile = vraiment facile »</strong> après La Circaète. FC plafonnée à 144, allure ≥ 6:00/km. Les lignes droites du footing technique (S25-1) sont l'exception : 6 accélérations courtes et relâchées, juste pour réveiller la foulée — pas un travail de vitesse, jamais en force pour préserver ton dos.</p>`},
 am:{titre:'Allure marathon',icone:'🎯',sections:[
   ...
   coach:`<p>Loïc, la S25-3 (1×4 km à allure marathon) est ta première vraie touche de spécificité du bloc. 4 km seulement, mais c'est un <strong>test de calibrage</strong> : est-ce que 5:20/km « coule » ou est-ce que ça force déjà ? Profites-en pour tester un gel (4Endurance ou Nduranz) même sur cette durée courte — c'est l'occasion de roder l'estomac. Au retour des USA, ces blocs AM passeront à 3×4 puis 2×6 km : c'est là que se jouera 3h45.</p>`},
 longue:{titre:'Sortie longue',icone:'⛰️',sections:[
   ...
   coach:`<p>Loïc, la S25-4 est une longue d'endurance pure (pas encore de finish AM, on est en reprise). Objectif : du temps de pied, FC ≤ 144, et surtout <strong>répéter le fueling</strong> dès maintenant même si tu n'as pas « besoin » — c'est un automatisme à réinstaller après ta défaillance de juin. Pars avec tes pastilles et un gel, et bois régulièrement dès le départ. Plus tard dans le bloc, ces longues iront jusqu'à 30 km avec 14 km de finish AM : la vraie séance reine de ta prépa.</p>`}
};
function theoryPanelHTML(tag){const T=RICH_THEORY[tag];if(!T)return '';
  const secs=T.sections.map((s,i)=>`<div class="theo-sec"><div class="theo-sec-h"><span class="theo-sec-n">${i+1}</span>${s.h}</div><div class="theo-sec-b">${s.html}</div></div>`).join('');
  return `<div class="theo-sheet-head"><div><div class="theo-kicker">Théorie & coaching · ${T.icone}</div><div class="theo-sheet-titre">${T.titre}</div></div><button class="theo-close" onclick="closeTheory()" aria-label="Fermer le panneau">✕</button></div>
    <div class="theo-sheet-body" id="theoSheetBody">${secs}<div class="theo-coachbox"><div class="theo-coachbox-h">🧠 Le mot du coach — personnalisé</div>${T.coach}</div><div class="theo-end">Tu sais maintenant le <em>pourquoi</em> et le <em>comment</em>. Bonne séance, Loïc.</div></div>`;
}
...
function closeTheory(){const ov=document.getElementById('theoOverlay');ov.classList.remove('ouverte');
